...
module.exports = function login () {
  ...
  return (req, res, next) => {
    ...
    var email = models.sequelize.escape(req.body.email || '');
    var password = models.sequelize.escape(insecurity.hash(req.body.password || ''));
    models.sequelize.query(`SELECT * FROM Users WHERE email = ${email} AND password = ${password} AND deletedAt IS NULL`, { model: models.User, plain: true })
